...
export async function POST(request: NextRequest) {
  // getSessionForDistributorArea instead of getDistributorSession: this endpoint
  // must stay reachable while mustChangePassword is set; disabled stays rejected.
  const result = await getSessionForDistributorArea()
  if (!result || result.disabled) return unauthorized()
