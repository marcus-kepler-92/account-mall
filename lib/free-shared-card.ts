...
/** 将免费共享卡 payload 格式化为可读文本（复制/展示用，仅账号密码，无 label） */
export function formatFreeSharedCardForCopy(payload: FreeSharedCardPayload): string {
...
const LABEL_PREFIXES: [string, keyof Pick<FreeSharedCardPayload, "account" | "password" | "region" | "birthday" | "securityAnswerFriend" | "securityAnswerWork" | "securityAnswerParents">][] = [
...
function parseByLabel(parts: string[]): FreeSharedCardPayload | null {
    const acc: Partial<FreeSharedCardPayload> = { region: "未知" }
...
function parseByHeuristic(parts: string[]): FreeSharedCardPayload | null {
...
export function parseCardContentWithDelimiter(
    content: string,
    delimiter?: string | null
): FreeSharedCardPayload | null {
    const trimmed = content.trim()
    if (!trimmed) return null

    const json = parseFreeSharedCardContent(trimmed)
    if (json) return json
