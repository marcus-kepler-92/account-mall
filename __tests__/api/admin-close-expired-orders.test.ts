...
const getSuperAdminSessionMock = getSuperAdminSession as jest.Mock
const closeExpiredOrdersMock = closeExpiredOrders as jest.Mock

describe("POST /api/admin/close-expired-orders", () => {
    beforeEach(() => {
        getSuperAdminSessionMock.mockReset()
        closeExpiredOrdersMock.mockReset()
    })

    it("returns 401 when not authenticated", async () => {
        getSuperAdminSessionMock.mockResolvedValue(null)
    ...
    it("returns { closed: 0, total: 0 } when no expired orders", async () => {
        getSuperAdminSessionMock.mockResolvedValue({ id: "admin_1" })
