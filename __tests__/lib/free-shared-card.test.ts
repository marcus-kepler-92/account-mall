...
describe("parseFreeSharedCardContent", () => {
  it("parses valid JSON with account and password", () => {
    const json = JSON.stringify({
      account: "test@apple.com",
      password: "secret123",
    });
    const result = parseFreeSharedCardContent(json);
  ...
  it("returns null for invalid JSON", () => {
    expect(parseFreeSharedCardContent("not json")).toBeNull();
    expect(parseFreeSharedCardContent("")).toBeNull();
  });

  it("returns null when account or password missing in JSON", () => {
    expect(parseFreeSharedCardContent(JSON.stringify({ account: "a" }))).toBeNull();
    expect(parseFreeSharedCardContent(JSON.stringify({ password: "p" }))).toBeNull();
  });
});
...
describe("isFreeSharedCard", () => {
  it("returns true for object with account and password", () => {
    const payload: FreeSharedCardPayload = {
      account: "a@b.com",
      password: "p",
      region: "未知",
    };
    expect(isFreeSharedCard(payload)).toBe(true);
  });

  it("returns true for parsed result", () => {
    const result = parseCardContentWithDelimiter("a@b.com pass");
    expect(result).not.toBeNull();
    expect(isFreeSharedCard(result!)).toBe(true);
  });

  it("returns false for object with only content", () => {
    expect(isFreeSharedCard({ content: "a@b.com:pass" })).toBe(false);
  });
});
