...
describe("dashboard-data", () => {
  ...
  function setupKpisMocks({
    ...
    productCount = 5,
    distributorCount = 2,
    pendingWithdrawal = { _count: { id: 3 }, _sum: { amount: 1200 } } as { _count: { id: number }; _sum: { amount: number | null } },
    pendingCommission = { _sum: { amount: 150 } } as { _sum: { amount: number | null } },
    settledCommission = { _sum: { amount: 200 } } as { _sum: { amount: number | null } },
    paidFee = { _sum: { feeAmount: 10 } } as { _sum: { feeAmount: number | null } },
    lastPeriodCommission = { _sum: { amount: 180 } } as { _sum: { amount: number | null } },
    lastPeriodFee = { _sum: { feeAmount: 8 } } as { _sum: { feeAmount: number | null } },
    thisPeriodCommission = { _sum: { amount: 160 } } as { _sum: { amount: number | null } },
    thisPeriodFee = { _sum: { feeAmount: 5 } } as { _sum: { feeAmount: number | null } },
    ...
    prismaMock.order.aggregate
      .mockResolvedValueOnce({
        ...
        _count: {
          id:
            statusGroups.find((s) => s.status === "COMPLETED")?._count.id ?? 20,
        },
        _avg: null,
        _min: null,
        _max: null,
  ...
  describe("getOrderStatusDistribution", () => {
    it("returns labels and counts for each status", async () => {
      prismaMock.order.groupBy.mockResolvedValueOnce([
        { status: "COMPLETED", _count: { id: 10 } } as any,
        { status: "PENDING", _count: { id: 2 } } as any,
        { status: "CLOSED", _count: { id: 1 } } as any,
      ]);
  ...
  describe("getDashboardData", () => {
    it("returns all sections in one call with new profit fields", async () => {
      const zeroAggregate = {
        _sum: { amount: null, feeAmount: null },
        _count: { id: 0 },
        _avg: null,
        _min: null,
        _max: null,
      ...
      prismaMock.withdrawal.aggregate.mockResolvedValue(zeroAggregate as any);
      ...
      expect(result.trend7.every((p) => typeof p.净收入 === "number")).toBe(
        true,
      );
    });
  });
