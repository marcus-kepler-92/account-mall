  ...
  if (process.env.SEED_E2E === "1") {
    ...
    try {
      ...
      if (pendingOrders.length > 0) {
        ...
        console.log(
          `  ✅ Closed ${pendingOrders.length} PENDING order(s) and released cards.`,
        );
      }

      // 清除 e2e-product 的折扣使用记录，确保 exit-discount 测试每次都能拿到 eligible:true
      const e2eProductForCleanup = await prisma.product.findUnique({
        where: { slug: "e2e-product" },
        select: { id: true },
      });
      if (e2eProductForCleanup) {
        const deleted = await prisma.exitDiscountUsage.deleteMany({
          where: { productId: e2eProductForCleanup.id },
        });
        if (deleted.count > 0) {
          console.log(
            `  ✅ Cleared ${deleted.count} ExitDiscountUsage record(s) for e2e-product.`,
          );
        }
      }
      ...
      if (lowStockNeed > 0) {
        ...
        console.log(
          `  ✅ E2E low-stock cards: ${lowStockNeed} added (total UNSOLD: 2).`,
        );
      } else {
