...
- **E2E**：`npm run test:e2e`（需先启动 DB 并完成 migrate，再执行 **`SEED_E2E=1 npm run db:seed`** 以创建 E2E 用商品与卡密；本地可先 `npm run dev` 或由 Playwright 自动启动。其中「完整支付流程」用例需配置z-pay四件套：`ZPAY_PID`、`ZPAY_KEY`、`ZPAY_SUBMIT_URL`、`ZPAY_SITE_NAME`，未配置时该用例会自动 skip。）
